...
def editing():
    ...
    try:
        what_edit = int(
            input('Что хотите изменить? (1 - название, 2 - содержание)'))
    except ValueError:
        print('Ошибка ввода')
        return
    ...
    with open('Note.csv', 'w', encoding='utf-8') as note:
        for i, line in enumerate(old_data):
            if line.split(';')[0] != edit_key:
                ...
                if what_edit == 1:
                    note_name = super_input(
                        "Новое название: ", inp=line_array[1])
                    note.write('{};{};{};{}\n'.format(
                        edit_key, note_name, line_array[2], date_note))
                    print('\nНазвание заметки №{} успешно изменено'.format(edit_key))

                elif what_edit == 2:
                    note_body = super_input(
                        "Новое название: ", inp=line_array[2])
                    note.write('{};{};{};{}\n'.format(
                        edit_key, line_array[1], note_body, date_note))
